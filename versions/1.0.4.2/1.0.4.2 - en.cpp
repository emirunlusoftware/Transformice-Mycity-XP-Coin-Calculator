...
#include "dialogs_en.h" //Brings dialog arrays here
using namespace std;
...
void General_Function(int seviye, int seviye2, int sonuc, int sonuc2, long long toplam, long long toplam2, long long xp, long long xp2, long long islem, int f)
{
...
int main()
{
	...
	int seviye, seviye2; unsigned int min; long long xp, xp2, avg, max, y;
main:
	cout << "\n\n" << menu[0] << "\n\n" << menu[1] << "\n" << menu[2] << "\n" << menu[3] << "\n\n" << menu[4] << "\n" << menu[5] << "\n\n" << menu[6] << "\n" << menu[7] << "\n" << menu[8] << "\n";
main_x:
	x = _getch();
	...
	main1:
		...
		while (1)
		{
			cout << calc_xp[1];
			cin >> x;
			if (!isNumber(x) || x.length() > 5 || stoi(x) < 1 || stoi(x) > 10000)
			{
				xp_error1();
				continue;
			}
		...
		while (1)
		{
			...
			cin >> x;
			if (!isNumber(x) || x.length() > 12 || stoll(x) < 0 || stoll(x) > LvlMaxXP(0, seviye))
			{
				xp_error2(); cout << LvlMaxXP(0, seviye) << "!" << "\n";
				continue;
			}
			xp = stoll(x);
			break;
	...
	main3:
		...
		while (1)
		{
			...
			if (!isNumber(x) || x.length() > 12 || stoll(x) < 0 || stoll(x) > LvlMaxXP(0, seviye2))
			{
				xp_error2(); cout << LvlMaxXP(0, seviye2) << "!" << "\n";
				continue;
			}
			xp2 = stoll(x);
			break;
	...
	main4:
		...
		while (1)
		{
			cout << "\n" << calc_coin[1] << "\n" << calc_coin[2];
			cin >> x;
			if (!isNumber(x) || x.length() > 6 || stoi(x) < 0)
			{
				coin_error();
				continue;
			}
