...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	HANDLE hMutex = CreateMutex(
		NULL, TRUE,
		L"Mutual Exclusion Handle prevents multiple instances of a program from running simultaneously."
	);
	...
	const wchar_t CLASS_NAME[] = L"Mycity XP & Coin Calculator Window";
	...
	if (!RegisterClass(&wc)) {
		MessageBox(NULL, L"RegisterClass(&wc) failed.", L"Error", MB_ICONERROR);
		return 1;
	}
	...
	if (!hwnd)
	{
		MessageBox(NULL, L"Program Handle WiNDow couldn't be created!", L"Error", MB_ICONERROR);
		return 1;
	}
